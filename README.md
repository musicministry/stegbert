...
Single liturgy posts -- the "Weekly lineups" -- can be created individually or a week at a time. Use `post-week.py` to simultaneously post all liturgies in the current week. Simply provide the calendar to process:

```bash
python ./scripts/post-week.py 2026
```

This will find and post any liturgies between the day the script is run and the next Sunday, exclusive. For example, if this script is run on Palm Sunday, Holy Thursday, Good Friday, and the Easter Vigil will be posted, but neither Palm Sunday nor Easter Sunday will be generated. Optionally, single-page PDFs can also be generated for the posts using the `--pdf` flag, but this does not happen by default to prevent unnecessary file clutter accumulating within the project.

This script currently runs automatically every Sunday afternoon as a GitHub Action to facilitate a hand-off approach to maintaining the site, but this behavior can be modified or supressed altogether in the `.github/workflows/post-week.yml` GitHub Actions workflow file.

Specific liturgies can also be posted manually using the `post.py` script. Simply pass the calendar year and either the date (formatted "YYYY-MM-DD") or the feast label (the names of the JSON blocks [created above](#creating-hymn-lists)) to the `-p, --publish` flag. For example:

```bash
python ./scripts/post.y 2026 --publish '2025-12-25'
```

or

```bash
python ./scripts/post.y 2026 --publish 'christmas_day'
```

Posts can also be created for special occassions that do not otherwise appear in the regular yearly music planner. These occasions should be added to the `occasions.py` file. To process one of these events:

```bash
python ./scripts/post.py 2026 -p 'occasions: confirmation'
```

where `occasions:` tells the script to look for the hymn scheduled `confirmation` in the `occasions.py` file.

A single-page PDF can also be created for the liturgy using the `--pdf` flag.

This script produces a Quarto Markdown file. Both this file and the accompanying PDF, if created, are named `YYYY-MM-DD-feast` by default, where "YYYY-MM-DD" is the date of the liturgy and "feast" is the feast label (the names of the JSON blocks [created above](#creating-hymn-lists)). This file name can be changed by passing a new file name to `-o, --outfile`. This is useful if you want to create a post or PDF for an occasion but you do not want it released on the website. This can be achieved by starting the file name with a leading underscore, which will cause Quarto to ignore that file when the site is updated. Note that the Quarto Markdown files, which create the website pages, are written to to the `posts/` directory while any PDF files are saved in `_site/files/` but are also accessible using the PDF download link on the accompanying web page, even if it's not publicly published.
...
The `Lookup` tab provides a simple utility for finding calendar events on the cantor calendar by name or Mass time. It uses a custom dataframe that must be updated manually in order to capture the latest version. The easiest way to update the utility is to use `cantors.py` with desired start and end date formatted "YYYY-MM-DD", between which events are pulled (inclusive of the start and end date):

```bash
python ./scripts/cantors.py --start 2026-01-01 --end 2026-02-28
```
...
`cantor-counts.py` is a helpful tool that reads in the CSV generated by `cantors.py` and prints a chronological list of events to the screen. It also tallies up the number of occurances of each event name (*i.e.*, cantor name) and prints the counts to the screen. This is useful for keeping track of how many times each cantor is assigned. Simply specify the CSV file to read in using `-i, --infile`, if different from the default, and, if desired, a month number (1-12) to filter by using `-m, --month`. For example:

```bash
python ./scripts/cantor-counts.py --month 3

CANTOR SCHEDULE FOR MARCH:

         Mass Time     Name
Date                       
Sun 3/1    8:00 AM   Joseph
Sun 3/1   11:00 AM     Mary
Sat 3/7    5:00 PM    Peter
Sun 3/8    8:00 AM     Paul
Sun 3/8   11:00 AM Veronica
Sat 3/14   5:00 PM    Sarah
Sun 3/15   8:00 AM   Joseph
Sun 3/15  11:00 AM    Sarah
