...
def buffer(price, percentage):
    if percentage > 1:
        return price * (percentage - 1)
    else:
        return price * percentage

def tax(price, tax=0.07):
    if tax > 1:
        return price * (tax - 1)
    else:
        return price * tax

def roundup(x, by=1000):
    return int(np.ceil(x / by) * by)

def display(x):
    return f"${format(x, ',.0f')}"
...
# 2025
imagine359L_2025_tax = tax(price=(imagine359L + platform), tax=sales_tax)
imagine359L_2025_buffer = buffer(price=(imagine359L + platform), percentage=buffer_pct)
imagine359L_2025_subtotal = imagine359L + platform + imagine359L_2025_buffer + imagine359L_2025_tax
imagine359L_2025_total = imagine359L + platform + imagine359L_2025_buffer

imagine359D_2025_tax = tax(price=(imagine359D + platform), tax=sales_tax)
imagine359D_2025_buffer = buffer(price=(imagine359D + platform), percentage=buffer_pct)
imagine359D_2025_subtotal = imagine359D + platform + imagine359D_2025_buffer + imagine359D_2025_tax
imagine359D_2025_total = imagine359D + platform + imagine359D_2025_buffer

infinity367L_2025_tax = tax(price=(infinity367L + platform), tax=sales_tax)
infinity367L_2025_buffer = buffer(price=(infinity367L + platform), percentage=buffer_pct)
infinity367L_2025_subtotal = (infinity367L * yearly_inc) + platform + infinity367L_2025_buffer + infinity367L_2025_tax
infinity367L_2025_total = (infinity367L * yearly_inc) + platform + infinity367L_2025_buffer

infinity367D_2025_tax = tax(price=(infinity367D + platform), tax=sales_tax)
infinity367D_2025_buffer = buffer(price=(infinity367D + platform), percentage=buffer_pct)
infinity367D_2025_subtotal = infinity367D + platform + infinity367D_2025_buffer + infinity367D_2025_tax
infinity367D_2025_total = infinity367D + platform + infinity367D_2025_buffer

# 2026
imagine359L_2026_tax = tax(price=((imagine359L * yearly_inc) + platform), tax=sales_tax)
imagine359L_2026_buffer = buffer(price=((imagine359L * yearly_inc) + platform), percentage=buffer_pct)
imagine359L_2026_subtotal = (imagine359L * yearly_inc) + platform + imagine359L_2026_buffer + imagine359L_2026_tax
imagine359L_2026_total = (imagine359L * yearly_inc) + platform + imagine359L_2026_buffer

imagine359D_2026_tax = tax(price=((imagine359D * yearly_inc) + platform), tax=sales_tax)
imagine359D_2026_buffer = buffer(price=((imagine359D * yearly_inc) + platform), percentage=buffer_pct)
imagine359D_2026_subtotal = (imagine359D * yearly_inc) + platform + imagine359D_2026_buffer + imagine359D_2026_tax
imagine359D_2026_total = (imagine359D * yearly_inc) + platform + imagine359D_2026_buffer

infinity367L_2026_tax = tax(price=((infinity367L * yearly_inc) + platform), tax=sales_tax)
infinity367L_2026_buffer = buffer(price=((infinity367L * yearly_inc) + platform), percentage=buffer_pct)
infinity367L_2026_subtotal = (infinity367L * yearly_inc) + platform + infinity367L_2026_buffer + infinity367L_2026_tax
infinity367L_2026_total = (infinity367L * yearly_inc) + platform + infinity367L_2026_buffer

infinity367D_2026_tax = tax(price=((infinity367D * yearly_inc) + platform), tax=sales_tax)
infinity367D_2026_buffer = buffer(price=((infinity367D * yearly_inc) + platform), percentage=buffer_pct)
infinity367D_2026_subtotal = (infinity367D * yearly_inc) + platform + infinity367D_2026_buffer + infinity367D_2026_tax
infinity367D_2026_total = (infinity367D * yearly_inc) + platform + infinity367D_2026_buffer
```

### BLUF / TL;DR

The organ at St. Egbert's is now more than 30 years old, making it vulnerable to part failures and ongoing repairs. Its sound sampling technology is outdated and uninspiring by modern standards, and the instrument itself is undersized for the church and the parish's music ministry program. The parish should begin proactively planning and raising money to replace it within the next few years, aiming to do so before it stops working to a degree that it is unfeasible to repair. Estimated project cost ranges from **`{python} display(roundup(imagine359D_2026_total))`** to **`{python} display(roundup(infinity367D_2026_total))`** (all inclusive) in 2026, with prices increasing roughly 16% each year---another reason to act sooner rather than later.

The recommended target organ for St. Egbert Church is a [**Rodgers Infinity 367D**](https://www.rodgersinstruments.com/organs/infinity-series/infinity-series-367) three-manual instrument at an estimated price of **`{python} display(infinity367D_2026_total)`** (cost breakdown [below](#what)).
...
:::: {.columns .content-visible when-format="html"}
::: {.column width=42.4%}
...
Related to [the size of the organ](#organ-is-too-small-for-liturgical-and-music-ministry-needs), the organ lacks a number of common features and functionality that assist the organist, enhance the user experience, and make the instrument behave as expected. Some of the more notable deficiencies:
...
The ideal organ for St. Egbert's is a [Rodger's Infinity 367D](https://www.rodgersinstruments.com/organs/infinity-series/infinity-series-367) to be installed by [R.A. Daffer Church Organs, Inc.](https://dafferorgans.com/), the Mid-Atlantic regional Rodgers distributor out of Jessup, MD. The estimated cost breakdown for 2026, which includes a 10-year warranty, is as follows:
...
| **LINE ITEM**                    | **COST** |   |
|:---------------------------------|---------:|---|
| Rodgers Infinity 367D organ      | `{python} display(infinity367D*yearly_inc)` |   |
| Rolling platform                 | `{python} display(platform)` |   |
| Buffer for parish expenses (10%) | `{python} display(infinity367D_2026_buffer)` |   |
| Sales tax                        | `{python} display(infinity367D_2026_tax)` |   |
| **Sub Total**                    | **`{python} display(infinity367D_2026_subtotal)`** |   |
| Sales tax reimbursement          | -`{python} display(infinity367D_2026_tax)` |   |
| **TOTAL**                        | **`{python} display(infinity367D_2026_total)`** |   |
...
![](https://www.rodgersinstruments.com/uploads/products/banners/fotos/large_foto/750x700/367---banner-1.png){fig-alt="Rodgers Infinity 367D digital organ" width="97%"}
...
::: {.callout-note title="A note on sales tax"}
North Carolina state sales tax is typically paid up front and later reimbursed to the parish.
:::
...
* Mechanical (moving) drawknobs that give the organ an authentic behavior and an attractive aesthetic appearance
...
* Lighted and adjustable music stand with mobile tablet (*e.g.*, iPad) support and automatic page-turn technology

The stop list is extremely versatile, with full principal, flute, and reed choruses in every division, plus a floating solo division that can be assigned to any manual. This makes the instrument very capable for accompanying congregational singing with a robust principal chorus from the Great division, cantors with soft yet articulate flutes from the Swell division, choir with supportive but softer principal chorus from the Choir division, and instruments with softer principal choruses from Choir or Swell division. The floating Solo division can be used on a moment's notice to help a struggling choir or congregation by articulating the melody line from any manual. Its four organ palettes (American Eclectic, English Cathedral, French Romantic, and German Baroque) offers immense flexibility and an infinite number of possible registrations. The large orchestral library supplements the instrument with dozens of string, brass, wind, reed, and percussion instruments that can be played either by themselves or along with the organ.

The Infinity supports up to 22 channels of audio. The current proposal from Rodgers includes 14 channels for St. Egbert's: ten would be used for the main organ, with speakers installed either in the rear of the church on either side of the stained glass window or (less likely) in the front on either side of the sanctuary. Unlike the current installation, which has only two channels, both of which are installed on one side of the church, this installation will allow the organ sound to be distributed evenly throughout the church without needing to overpower the speakers with excessive volume. In addition to these ten channels, the proposal calls for a four-channel antiphonal organ to be installed in the choir area. This will likely play only the Choir and Solo divisions to help the choir and instruments hear the organ in real time (without sound delays caused by the main organ speaking from the middle of the church). Importantly, the antiphonal organ will be voiced at a much lower volume than the main organ, providing only what is needed for musician support, not for the entire congregation. This setup resembles how a pipe organ would be installed in the church and will constitute a remarkable improvement over the current situation.
...
The Infinity 367D is the ideal instrument and its price should be considered the target amount to raise. While financing is available, this is discouraged in favor of taking the necessary time to raise the needed funds to avoid taking on debt. If, after some period of time, fundraising efforts plateau and fall short of the target, [other options](#analysis-of-alternatives) can be considered based on the amount raised. 

Most people and parishes do not give much thought to the monetary value of an organ until it comes time to replace one, so the cost of a new organ can sometimes invoke sticker shock. Parishioners should be reminded that a comparable pipe organ would cost much more. Some fundraising strategies that have proven successful in other parishes include:
...
2. **Fundraising efforts:** Creative fundraising projects, like buying organ keys, pedals, or stops in memory of loved ones, can be effective for the average parishioner. A sample flyer and pledge sheet, made for another parish but easily adaptable to St. Egbert's, are appended as example advertisements.
...
One advantage to replacing the organ before it no longer functions is that the parish could consider selling it to help offset the price of the new one. Otherwise, the dealer will take it, possibly with a slight "trade in" discount, like when trading in a used car for a new one. There is a market for used organs, either for small churches or home practice organs. For example, a slightly newer Rodgers digital organ of similar size but with considerably more features than St. Egbert's current organ recently sold for $2,600 on Facebook Marketplace. St. Egbert's organ has an internal speaker, making it entirely self-contained, which could make it more attractive for a home practice organ. 

### Analysis of Alternatives

#### Rodgers options

The next model down from the Rodgers Infinity 367 is the [Imagine 359](https://www.rodgersinstruments.com/organs/imagine-series/imagine-359d). A smaller yet equally capable organ, the Imagine 359D offers 51 stops, 147 organ voices, and 45 orchestral voices. It contains the same highly authentic sound technology but lacks many features and additional flexibilities of the Infinity 367. For example, in addition to fewer stops, it lacks the floating Solo division, mobile tablet integration with digital page turning, and a number of convenient pistons for controlling the organ itself. Priced at about $`{python} int(roundup(imagine359D_2026_total)/1e3)`k, which includes the same 14-channel audio installation [described above](#why), this organ would still be a remarkable improvement and upgrade from the existing organ. The cost estimate breaks down as follows:

| **LINE ITEM**                    | **COST** |   |
|:---------------------------------|---------:|---|
| Rodgers Infinity 359D organ      | `{python} display(imagine359D*yearly_inc)` |   |
| Rolling platform                 | `{python} display(platform)` |   |
| Buffer for parish expenses (10%) | `{python} display(imagine359D_2026_buffer)` |   |
| Sales tax                        | `{python} display(imagine359D_2026_tax)` |   |
| **Sub Total**                    | **`{python} display(imagine359D_2026_subtotal)`** |   |
| Sales tax reimbursement          | -`{python} display(imagine359D_2026_tax)` |   |
| **TOTAL**                        | **`{python} display(imagine359D_2026_total)`** |   |
: Cost estimate of a Rodgers Infinity 359D organ installation based on 16% increase from 2025 prices, which represents the 2024-2025 price increase. Prices can be locked in for a period of time (usually up to one year) with the signing of a contract and initial deposit.

Priced in between the Infinity 367D and the Imagine 359D is the Infinity 367L ($`{python} int(roundup(infinity367L_2026_total)/1e3)`K), which is exactly the same as the 367D except it has lighted drawknobs instead of mechanical drawknobs. Mechanical drawknobs are more realistic than lighted, since pipe organs are mechanical instruments.
...
4. **Voicing:** Voicing is when the sound of every note of every stop is carefully adjusted and fine-tuned for the room in which the organ is installed. This is a very important part of an organ installation, as it is what makes the instrument sound its best. Allen organs, if they're voiced at all, are often voiced by service technicians rather than organists. Rodgers uses organists to professionally voice their instruments, a detail that makes a big difference.
...
#### Pre-owned (used) organ

Pre-owned organs, whether purchased as a private sale or through a commercial sales company like R.A. Daffer Church Organs, often cost significantly less than new organs, but there are several reasons why churches should be leery of this idea:

1. **Age:** As discussed [above](#overview), digital organs generally have a finite usable life span. Assume, for the sake of argument, this lifespan is (generously) 50 years. With very rare exceptions, the newest used organs available at any time are usually 20-25 years old. That currently puts most from the early 2000s, which means they are only about 10 years newer than St. Egbert's current organ. From a human psychologically perspective, parishioners think they are getting a new organ, and while it is indeed new to them, a 20-25 year old organ is halfway through its reliable lifespan. That means the parish will find itself in the market again in another 20 years or so, which is not a long time. Most parishioners will then still remember the previous purchase and think, "We just bought a new organ, why do we need another one?"

2. **Old technology:** Rodgers and Allen tend to update their model offerings roughly every 10-15 years. This is when the largest technological advances get rolled out. From a sound technology standpoint, only organs built within the last 15 years should be seriously considered. It is within this time frame that the modern sampling capabilities became widely available, and the sound quality is leaps and bounds ahead of organs from even the previous decade. The industry has now reached a point where any new improvements in sound quality capabilities are unlikely to be perceivable by the human ear, just like what has happened in the television industry. It is advisable to purchase the current state-of-the-art.

3. **No warranty:** Pre-owned organs are usually sold "as is" without any warranty on the organ itself, though commercial organ distributors will sometimes include limited warranties on installation. Private sales will never have warranties.
...
Virtual organs consist of custom consoles that either contain drawknobs or tabs that control the software, or touch screen monitors that the organist uses to engage or disengage stops. They are designed such that the computer plugs directly into them and the software can be configured to be controlled by all of the stops, tabs, pistons, and shoes on the console.

While these organs cost a fraction of the price of a digital organ, they are are still rarely found in churches. There are several critical limitations and concerns. First is user experience. A computer-savvy organist may have little trouble adjusting to using a touch screen monitor to control the instrument, but a classically trained organist will undoubtedly find herself out of her element at such an instrument. It may, therefore, be difficult to find someone to play it in the future. Second, they lack the same level of support that one would get from a major manufacturer like Rodgers or Allen. Consoles, speakers, amplifiers, wires, computer, and software must be purchased separately and installed by the church. While not impossible to accomplish with the right expertise in-house, this piecemeal setup introduces many potential points of failure and incompatibility. Third, software running on a computer is inevitably prone to software and hardware issues -- crashes, operating system and software updates, computer slowing down over time. A digital organ powers up immediately when turned on, but a virtual organ needs to boot an operating system and load the software. This is not a problem for casual settings like practicing at home, but it raises concerns in a liturgical setting where dependability is paramount. Overall, while promising, this technology is considered to not yet be mature enough for installation in a church like St. Egbert's.

#### Pipe organ

Pipe organs are laudable investments: nothing is more authentic than the "real deal," and properly maintained, they will outlive their building. Pipe organs generally cost many times as much as a comparable digital organ, and require regular maintenance, including tuning twice a year. A new custom pipe organ for St. Egbert's would likely cost 5-10x as much as the Infinity (*i.e.*, $1-2M.)

There are also used pipe organs available, for example, through [Organ Clearing House](https://www.organclearinghouse.com/). Some are even free so that the buyer need "only" pay for dismantling, moving, and installation. In some cases, like [this historic Aeolian-Skinner organ](https://www.organclearinghouse.com/organs-for-sale#/3032-aeolianskinner-boston-ma), are already dismantled and out of its original loft, which is a large cost itself. They are usually in varying degrees of (dis)repair, requiring a partial or complete refurbishing to get them functional again. Additional costs include retrofitting its new home, such as electrical, structural, architectural modifications. The cost of all of this depends primarily on the size of the instrument and its condition, but will almost always be less expensive than the same instrument brand new, since so much of the cost of a pipe organ is in parts and material.

It has been assumed that a full pipe organ, whether new or used, is out of reachable budget for St. Egbert's. If the parish wishes to go down this route, more research will need to be done and new proposals will need to be sought. It is worth noting that new pipe organs are always custom-built according to desired specifications and room considerations. Because of this, a pipe organ that is plenty adequate for the parish's needs yet is also smaller than the Rodgers Infinity 367D could easily be designed, so a $2.5M (10x) price estimate could be unnecessarily high.

### Summary

The Rodgers Infinity 367D organ is a remarkably impressive instrument that would be a tremendous upgrade from the current organ at St. Egbert's. With three manuals, 67 stops, more than 500 organ voices, and 14 audio channels divided between a ten-channel main organ and a four-channel antiphonal organ, this organ is plenty adequate for the size of the church and the needs of the existing music ministry. With the current organ beginning to show signs of its 32 years of service, the parish is encouraged to proactively begin raising funds now so that it can replace the instrument before it dies completely. While the Infinity's price tag of `{python} display(infinity367D_2026_total)` will require significant fundraising, it will constitute a dedicated investment in the future of the parish: its stunning fidelity to the sounds of pipe organs combined with endless possibilities of stop combinations and orchestral integrations will undoubtedly raise the parish's music program to a new level for decades to come.
